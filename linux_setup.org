...
The slack plugin for weechat can be setup inside of a weechat instance with
#+begin_src shell :cache no :results output
cd ~/.weechat/python
...
If loading the script doesn't work it might be because of missing python
dependencies.
#+begin_src shell :cache no :results object
...
:   libllvm6.0 linux-headers-4.15.0-1030-oem linux-image-4.15.0-1030-oem
:   linux-modules-4.15.0-1030-oem linux-oem-headers-4.15.0-1030
...
found on reddit]].
#+begin_src shell :tangle "~/.qutebrowser/userscripts/org-capture" :tangle-mode (identity #o755)
#!/usr/bin/env bash
